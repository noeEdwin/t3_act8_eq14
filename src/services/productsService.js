  ...
  try {
    const response = await fetch(`${BASE_URL}/add`, {
      ...
      body: JSON.stringify({
        ...
        description: productData.description ? productData.description.trim() : 'Producto registrado en catálogo Aura Hogar',
      }),
    });
...
export async function updateProduct(productId, productData) {
  ...
  try {
    ...
    const updatedProduct = await handleResponse(response, `No se pudieron guardar los cambios en el producto #${productId}.`);
...
export async function deleteProduct(productId) {
  ...
  try {
    ...
    const deletedData = await handleResponse(response, `No se pudo eliminar el registro del producto #${productId}.`);
    return deletedData;
  } catch (error) {
